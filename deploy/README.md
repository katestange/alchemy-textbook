...
0. **No `.tex` yet?** A professor starting from handwritten/scanned course
   notes can transcribe them to portable LaTeX with the AI prompts in
   `notes-to-latex.md` (repo root): Prompt A for any chat website (they
   crop figures by hand), Prompt B for a coding agent like Claude Code
   (it crops figures and compiles by itself).
