...
| 83 | **Adaptability: book.toml config layer + one-instance-per-book deployment (department-adoption goal).** Everything identifying *which book this is* moved to **`book.toml`** (title/subject/voice → `CORE_SYSTEM_PROMPT`; slug → namespaced localStorage; source .tex/images/split level → pipeline; solution/question env names → `solutions.py`; invite prefix → `/admin/codes`; Desmos key → applets), served to the reader via `GET /api/book` (no branding in the frontend bundle). **`adopt-book.sh`** onboards a new book (toolchain check → build → accept baselines → frontend build → handoff steps); the stale `assert len(blocks)==817` test became invariants. New env surface: `TEXTBOOK_HOST/PORT/BUILD_DIR/COOKIE_SECURE/SEED_DEV_CODE` (dev code no longer seeded by default; cookies Secure-flagged under TLS). **`deploy/`** = Dockerfile (book baked at image build, Debian 12's LaTeXML 0.8.7), one-service-per-book compose, per-subdomain nginx TLS template, ops doc. **Multi-tenancy rejected** in favor of one container/DB/key/admin per book (billing + moderation isolation for free). Remaining generalization (preprocessor shim catalog, multi-file .tex, book-class/`\chapter`, bibliography, builtin-applets-as-data) is planned in **`ADAPTABILITY.md`** (Q7/Q8) | 2026-07-09 |
| 84 | **Usage stats: richer collection + `/admin/stats` panel (author request: "are you tracking creature usage?").** Collection closed four gaps, all pseudonymous (no per-student drill-down): (1) `usage_log` gained `model` / `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` (NULL on the disconnect-estimate path) and an `ephemeral` flag (`[[EPHEMERAL]]` responses — charged but never stored); (2) `cached_content.served_count` counts each time an artifact is **surfaced at selection** (bumped in `GET /api/content/{hash}` when rows are returned) — the "generated once, served many" cache-leverage number; (3) new `chapter_views` table (day × chapter × signed-in/anon tallies, bumped in `GET /chapter/{n}` — counts only, no identity); (4) instructor-ledger rows also record the model. **`/admin/stats`** reads it for two audiences: *pitch* numbers (total/per-active-student spend, cache leverage ×, library size, chapter views) and *teaching* signals (ToC-to-subsection tree with rolled-up usage bars, top help-request sections with creature mix as the diagnostic, per-creature table, 14-day activity, most-surfaced artifacts). Verified end-to-end on a scratch DB (Q3/Q5) | 2026-07-09 |
| 85 | **Classic chips kept as a permanent, instructor-selectable alternative to creature art (author request: "some people may not like the creative version").** The Decision-56 quiet-mode colored chips are not a stopgap to be deleted when the art ships — they stay as the "classic" look. New setting `creature_art` (`on`/`off`, default `on`), chosen on **`/admin/settings`** (resurrected as a real page — amends the Decision-77 redirect-to-Solutions, which existed only because no general settings remained) and served to the reader in `GET /api/book` as `creature_art: bool`; the frontend stamps `<html data-creature-art="on|off">`. **Contract for the future art layer:** it must check this flag and stand down (leave the chips untouched) when `off`; until art is installed both modes render the chips. Distinct from the per-reader accessibility quiet mode (Q11) — this is the instructor's book-wide default (Q11/Q13) | 2026-07-09 |
