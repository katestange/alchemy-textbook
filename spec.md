...
| -- | PLANNED (author request 2026-07-06, queued behind the algebra pass since both edit the .tex): **AI-written solutions for every question + Quick Check.** Solutions live in the LaTeX (an `\begin{aisolution}` env, marked "AI Generated; needs review"), so a direct LaTeX compile shows them; LaTeXML renders them to `.ai-solution` markup that the browser hides behind a click. An admin dashboard toggle governs availability: OFF = solutions stripped server-side entirely; ON = served behind click-to-show | 2026-07-06 |
