...
| 76 | **Delete-forever for the creator.** Every AI artifact box carries a red "delete" control (distinct from the non-destructive per-reader "hide"); `DELETE /api/content/{id}` purges the row for everyone but is enforced creator-only (created_by_code must match; 403 otherwise). Complements moderation: a student can purge their own bad output so it never reaches the review queue. Shown only on own content (fresh generations, and hydrated artifacts where `own`); content id is stamped on the box at generation `done`/hydration. Applies to eureka chips too (Q3, author feedback) | 2026-07-06 |
